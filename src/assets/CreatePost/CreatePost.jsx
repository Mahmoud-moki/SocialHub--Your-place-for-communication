import { Avatar, Button, Card, Textarea } from '@heroui/react'
import { IconBulb, IconContrast2, IconSend } from '@tabler/icons-react'
import { useMutation, useQuery } from '@tanstack/react-query';
import React from 'react'

export default function CreatePost({ user }) {




  




  // if (!user) return <Card className="bg-white shadow p-5 rounded-2xl">Loading...</Card>;

  return (
    <Card className="bg-white shadow p-5 rounded-2xl space-y-4">
      <header className="flex items-center gap-3">
        <Avatar
          src={user?.photo}
          className="w-12 h-12 border border-gray-400/20"
        />
        <div>
          <h2 className="font-semibold">Create a Post</h2>
          <p className="text-default-400 text-sm -mt-1">
            Share your thoughts with the world
          </p>
        </div>
      </header>
      <form className="space-y-4">
        <div className="relative">
          <Textarea
            placeholder="What's on your mind?"
            name="body"
            className="pl-11 min-h-24"
          />
          {/* <IconBulb className="absolute left-4 top-1/2 -translate-y-1/2 text-default-400" /> */}
        </div>
        <div className="flex justify-between items-center border-t border-gray-400/20 pt-4">
          <label htmlFor="image">
            <div className="btn flex gap-1 items-center cursor-pointer">
              <IconContrast2 />
              <span>Photo</span>
            </div>
            <input
              type="file"
              id="image"
              name="image"
              className="hidden"
            />
          </label>
          <Button
            type="submit"
            className="flex gap-1 items-center bg-linear-to-br from-blue-500 to-cyan-400 text-white border-none disabled:opacity-70"
          >
            <span>Post</span>
            <IconSend />
          </Button>
        </div>

      </form>
    </Card>)
}
